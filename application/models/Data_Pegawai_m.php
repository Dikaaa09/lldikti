...
class Data_Pegawai_m extends CI_Model 
{
    ...
    public function get_kgb_terhapus($id = null)
    {
        $this->db->from('kgb_terhapus');
        $this->db->join('user', 'user.user_id = kgb_terhapus.user_id');
        $this->db->select('kgb_terhapus.*, user.name as user_name');
        if ($id != null) {
            $this->db->where('id_terhapus', $id);
        }
        $query = $this->db->get();
        return $query;
    }

    public function del_kgb($id)
	{
        $this->db->where('id_terhapus', $id);
        $this->db->delete('kgb_terhapus');
	}
}